namespace AsyncAwait {
	...
	function showOrderSummary(
		...
		const orderSummaryPromise: Promise<{ totalAmount: number }> =
	...
	async function manageOrder() {
		try {
			...
			const paymentData = await proceedToPayment(orderData.orderId);
			const amountInfo = await showOrderSummary(paymentData.data);
			const updatedWallet = await updateWallet(amountInfo.totalAmount);
